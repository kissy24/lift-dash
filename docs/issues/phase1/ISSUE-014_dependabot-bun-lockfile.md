...
- [x] 現在の`main`で発生している`bun install --frozen-lockfile`失敗が解消される
- [x] Dependabot PRに対して`bun.lock`更新用workflowが実行される
- [x] workflowはDependabot PRだけを対象にする
- [x] lockfile更新は`bun install --lockfile-only --ignore-scripts`に限定する
- [x] `bun.lock`に差分がある場合だけDependabot PRブランチへcommit/pushする
- [x] 通常のQuality CIは引き続き`bun install --frozen-lockfile`で検証する
- [x] 対応するテスト・検証がすべてグリーン
