...
- [x] `/dashboard`で期間フィルターを選択できる
- [x] 期間は直近30日 / 90日 / 180日 / 1年 / 全期間から選択できる
- [x] `/dashboard`で全種目または特定種目を選択できる
- [x] フィルター状態をURL queryで表現できる
- [x] 不正なqueryは安全な初期値へフォールバックする
- [x] フィルター変更後もグラフ・自己ベストカードが一貫したデータを表示できる
- [x] 対応するテストがすべてグリーン
